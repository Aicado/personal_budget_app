...
**Learning:** Parallelizing I/O-bound LLM calls using `asyncio.gather` in synchronous database methods (via `loop.run_until_complete` and `nest_asyncio`) reduces ingestion latency by ~80% for unique payees. However, intra-batch category consistency (LLM learning from its own choices within a single batch) is lost when calls are concurrent.
**Action:** Use `asyncio.gather` for LLM calls during ingestion to maximize throughput, and pass a static snapshot of known categories to maintain consistency across the batch.
