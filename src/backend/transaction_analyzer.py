...
class TransactionAnalyzer:
    ...
    def get_category_totals(self, df: pl.DataFrame | None = None) -> Dict[str, float]:
        ...
        # Using dict(zip()) is significantly faster than a list comprehension over to_dicts()
        # for constructing a dictionary from Polars columns.
    ...
    def get_summary_stats(self, df: pl.DataFrame | None = None) -> Dict[str, Any]:
        ...
        # This replaces a multi-pass approach (select + group_by) with a truly vectorized single pass.
        stats = df.select(
            [
        ...
        # Calculate averages based on total and unique months for O(1) derivation from O(n) scan
