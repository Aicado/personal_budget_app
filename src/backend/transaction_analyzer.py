...
class TransactionAnalyzer:
    ...
    def get_category_totals(self, df: pl.DataFrame | None = None) -> Dict[str, float]:
        ...
        # Using zip for vectorized dictionary construction is faster than to_dicts()
    ...
    def get_summary_stats(self, df: pl.DataFrame | None = None) -> Dict[str, Any]:
        ...
        # Truly single-pass aggregation for all stats to avoid scanning the same columns multiple times
        # and removing redundant group_by operations.
        stats = df.select(
            [
