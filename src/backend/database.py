...
class TransactionDatabase:
    ...
    def _initialize_db(self):
        ...
        # Create index on payee_mappings(payee) for efficient ingestion lookups
        try:
            self.conn.execute("""
    ...
    def insert_transactions(
        ...
        # Filter by payees in the current batch to improve performance and scalability
        unique_batch_payees = df["payee"].unique().to_list()
        mappings_df = pl.from_arrow(
            ...
            """, [unique_batch_payees]).fetch_arrow_table()
        )
        ...
        if categorizer:
            ...
            if not needs_llm_df.is_empty():
                ...
                # Pre-apply nest_asyncio once to allow nested event loops
                nest_asyncio.apply()
                ...
                async def categorize_all():
                    ...
                    async with httpx.AsyncClient(timeout=60.0) as client:
                        tasks = []
                        # Take a snapshot of categories to pass to all concurrent tasks
                        current_cats = list(all_categories)

                        async def categorize_one(row):
                            payee = row["payee"]
                            async with sem:
                                try:
                        ...
                        for row in unique_uncat_payees:
                            tasks.append(categorize_one(row))
                ...
                # Execute parallel categorization
    ...
    def get_account_summaries(self) -> List[Dict[str, Any]]:
        """Alias for get_files_info to maintain compatibility with main.py."""
