    ...
    def insert_transactions(self, df: pl.DataFrame, filename: str, account_name: str = None, account_type: str = None, account_path: str = None, categorizer=None) -> Dict[str, Any]:
        ...
        if categorizer:
            ...
            if not needs_llm_df.is_empty():
                ...
                # Parallelize LLM categorization to improve performance
                import nest_asyncio
                ...
                llm_mappings = {}
                # Take a snapshot of categories once for all concurrent tasks
                    ...
                    async with semaphore:
                        try:
                            llm_result = await categorizer.categorize_transaction(
                                payee, row["amount"], str(row["date"]), current_cats
                            )
                            if llm_result:
                ...
                for payee, llm_result in results:
                    if llm_result:
                        ...
                        # Add to known categories to help LLM stay consistent for future batches
