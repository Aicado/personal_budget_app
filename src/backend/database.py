    ...
    def insert_transactions(
        ...
        if categorizer:
            ...
            if not needs_llm_df.is_empty():
                ...
                # Parallelize LLM categorization using asyncio.gather and a semaphore to limit concurrency.
                # This significantly reduces ingestion latency when many unique payees need categorization.
                import nest_asyncio
                ...
                async def process_payee(row):
                    async with semaphore:
                        payee = row["payee"]
                        try:
                            # Pass a snapshot of categories; they don't need to be perfectly up-to-date for individual calls.
                            llm_result = await categorizer.categorize_transaction(
                                payee, row["amount"], str(row["date"]), list(all_categories)
                            )
                ...
                for payee, llm_result in results:
                    if llm_result:
                        ...
                        # Update local category list for subsequent processing if needed
                        if llm_result["category"] not in all_categories:
                            all_categories.append(llm_result["category"])

                if new_mappings_to_save:
                    # Bulk save payee mappings for performance
                    mappings_df = pl.DataFrame(new_mappings_to_save)
                    self.conn.register("new_mappings", mappings_df.to_arrow())
                    self.conn.execute("""
                        INSERT INTO payee_mappings (payee, category, category_group, confidence, created_at)
                        SELECT payee, category, category_group, confidence, CURRENT_TIMESTAMP
                        FROM new_mappings
                    ...
                    """)
                    self.conn.unregister("new_mappings")
