    ...
    def insert_transactions(
        ...
        if categorizer:
            ...
            if not needs_llm_df.is_empty():
                ...
                # Parallelize LLM categorization using asyncio.gather
                import nest_asyncio

                nest_asyncio.apply()

                async def get_categorization(row, sem):
                    async with sem:
                        payee = row["payee"]
                        try:
                ...
                tasks = [get_categorization(row, sem) for row in unique_uncat_payees]
                ...
                for payee, llm_result in results:
                    if llm_result:
                        ...
                        # Add to known categories for consistency within this batch (approximate)
