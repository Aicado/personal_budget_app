    ...
    def insert_transactions(self, df: pl.DataFrame, filename: str, account_name: str = None, account_type: str = None, account_path: str = None, categorizer=None) -> Dict[str, Any]:
        ...
        if categorizer:
            ...
            if not needs_llm_df.is_empty():
                ...
                # Parallelize LLM calls using asyncio.gather and a Semaphore
                import nest_asyncio
                nest_asyncio.apply()
                ...
                # Snapshot categories to avoid race conditions during parallel execution
                current_categories = list(all_categories)
                    ...
                    async with semaphore:
                        try:
                            llm_result = await categorizer.categorize_transaction(
                                payee, row["amount"], str(row["date"]), current_categories
                            )
                            if llm_result:
                ...
                for payee, llm_result in results:
                    if llm_result:
                        ...
                        # Add to known categories for subsequent files/batches
