    ...
    def insert_transactions(self, df: pl.DataFrame, filename: str, account_name: str = None, account_type: str = None, account_path: str = None, categorizer=None) -> Dict[str, Any]:
        ...
        if categorizer:
            ...
            if not needs_llm_df.is_empty():
                ...
                async def get_mapping(payee, amount, date, categories):
                    try:
                        result = await categorizer.categorize_transaction(payee, amount, date, categories)
                        return payee, result
                    except Exception as e:
                        print(f"LLM Categorization failed for {payee}: {e}")
                        return payee, None

                # Create tasks for all unique payees and run in parallel
                tasks = [
                    get_mapping(row["payee"], row["amount"], str(row["date"]), list(all_categories))
