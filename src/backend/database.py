    ...
    def insert_transactions(
        ...
        if categorizer:
            ...
            if not needs_llm_df.is_empty():
                ...
                # Parallelize LLM calls to significantly reduce latency
                # Use a semaphore to avoid overloading the LLM service (especially if local Ollama)
                semaphore = asyncio.Semaphore(10)

                async def run_categorization(row):
                    payee = row["payee"]
                    async with semaphore:
                        try:
                            # Pass a snapshot of categories to ensure consistency within the parallel batch
                ...
                tasks = [run_categorization(row) for row in unique_uncat_payees]
